...
@section('title', 'Gestión de Escuelas - Route Tracker')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Gestión de Escuelas</h1>
                <p class="text-gray-600">Administración de instituciones educativas</p>
            </div>
        ...
        <div class="px-6 py-4">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Nombre, código DANE..." class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
                <div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Código DANE</th>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    ...
                                    {{ $school->has_transport_service ? 'Sí' : 'No' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                ...
                                <div class="flex space-x-2">
                                    ...
                                    @if($school->school_status === 'active')
                                        <form action="{{ route('schools.suspend', $school) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-yellow-600 hover:text-yellow-900" onclick="return confirm('¿Estás seguro de suspender esta escuela?')">
                                                Suspender
                                            </button>
                                        </form>
                                    @else
                                        <form action="{{ route('schools.activate', $school) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-green-600 hover:text-green-900" onclick="return confirm('¿Estás seguro de activar esta escuela?')">
                                                Activar
                                            </button>
                                        </form>
                                    @endif
