...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Editar Suscripción</h1>
                <p class="text-gray-600 mt-2">Modifica la suscripción #{{ $subscription->subscription_id }}</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('subscriptions.show', $subscription) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition duration-200">
            ...
            <form method="POST" action="{{ route('subscriptions.update', $subscription) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="contract_id" id="contract_id" value="{{ old('contract_id', $subscription->contract_id) }}"
                        ...
                        <div>
                            ...
                            <select name="subscription_status" id="subscription_status"
                                    ...
                                    required>
                                <option value="">Seleccionar...</option>
                                <option value="active" {{ old('subscription_status', $subscription->subscription_status) === 'active' ? 'selected' : '' }}>Activo</option>
                                <option value="paused" {{ old('subscription_status', $subscription->subscription_status) === 'paused' ? 'selected' : '' }}>Pausado</option>
                                <option value="cancelled" {{ old('subscription_status', $subscription->subscription_status) === 'cancelled' ? 'selected' : '' }}>Cancelado</option>
                                <option value="expired" {{ old('subscription_status', $subscription->subscription_status) === 'expired' ? 'selected' : '' }}>Expirado</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="price_snapshot" id="price_snapshot" value="{{ old('price_snapshot', $subscription->price_snapshot) }}"
                                   step="0.01" min="0"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                   required>
                        ...
                        <div>
                            ...
                            <input type="number" name="platform_fee_rate" id="platform_fee_rate" value="{{ old('platform_fee_rate', $subscription->platform_fee_rate) }}"
                        ...
                        <div>
                            ...
                            <input type="date" name="next_billing_date" id="next_billing_date"
                                   value="{{ old('next_billing_date', $subscription->next_billing_date ? $subscription->next_billing_date->format('Y-m-d') : '') }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                   required>
                        </div>
                    </div>
                ...
                <div class="mt-8 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="payment_plan_type" id="payment_plan_type"
                                   value="{{ old('payment_plan_type', $subscription->payment_plan_type) }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="payment_plan_name" id="payment_plan_name"
                                   value="{{ old('payment_plan_name', $subscription->payment_plan_name) }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="payment_plan_description" id="payment_plan_description" rows="3"
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('payment_plan_description', $subscription->payment_plan_description) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="discount_rate" id="discount_rate"
                                   value="{{ old('discount_rate', $subscription->discount_rate) }}"
                                   step="0.01" min="0" max="100"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="payment_method" id="payment_method"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="">Seleccionar...</option>
                                <option value="credit_card" {{ old('payment_method', $subscription->payment_method) === 'credit_card' ? 'selected' : '' }}>Tarjeta de Crédito</option>
                                <option value="debit_card" {{ old('payment_method', $subscription->payment_method) === 'debit_card' ? 'selected' : '' }}>Tarjeta Débito</option>
                                <option value="bank_transfer" {{ old('payment_method', $subscription->payment_method) === 'bank_transfer' ? 'selected' : '' }}>Transferencia Bancaria</option>
                                <option value="cash" {{ old('payment_method', $subscription->payment_method) === 'cash' ? 'selected' : '' }}>Efectivo</option>
                                <option value="pse" {{ old('payment_method', $subscription->payment_method) === 'pse' ? 'selected' : '' }}>PSE</option>
                            </select>
                        </div>
                ...
                <div class="mt-8 flex justify-end space-x-3">
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-save mr-2"></i>Actualizar Suscripción
                    </button>
                </div>
            </form>
        </div>
