{{--
    Archivo: resources/views/admin/students/edit.blade.php
    Roles: admin
    Rutas necesarias: Route::get('admin/students/{student}/edit', [StudentController::class, 'edit'])
--}}

@extends('layouts.app')

@section('title', 'Editar Estudiante - Route Tracker')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Editar Estudiante</h1>
                <p class="text-gray-600">Modificar información del estudiante: {{ $student->given_name }} {{ $student->family_name }}</p>
            </div>
    ...
    <div class="bg-white shadow-md rounded-lg">
        ...
        <form action="{{ route('admin.students.update', $student) }}" method="POST" class="px-6 py-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="given_name" id="given_name" value="{{ old('given_name', $student->given_name) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="family_name" id="family_name" value="{{ old('family_name', $student->family_name) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="identity_number" id="identity_number" value="{{ old('identity_number', $student->identity_number) }}"
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="parent_id" id="parent_id" required
                            ...
                            @foreach($parents as $parent)
                                <option value="{{ $parent->parent_id }}" {{ old('parent_id', $student->parent_id) == $parent->parent_id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="grade" id="grade" required
                            ...
                            <option value="1" {{ old('grade', $student->grade) == '1' ? 'selected' : '' }}>1° Grado</option>
                            <option value="2" {{ old('grade', $student->grade) == '2' ? 'selected' : '' }}>2° Grado</option>
                            <option value="3" {{ old('grade', $student->grade) == '3' ? 'selected' : '' }}>3° Grado</option>
                            <option value="4" {{ old('grade', $student->grade) == '4' ? 'selected' : '' }}>4° Grado</option>
                            <option value="5" {{ old('grade', $student->grade) == '5' ? 'selected' : '' }}>5° Grado</option>
                            <option value="6" {{ old('grade', $student->grade) == '6' ? 'selected' : '' }}>6° Grado</option>
                        </select>
                        @error('grade')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="shift" id="shift"
                            ...
                            <option value="evening" {{ old('shift', $student->shift) == 'evening' ? 'selected' : '' }}>Noche</option>
                        </select>
                        @error('shift')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <textarea name="address" id="address" rows="3"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('address') border-red-300 @enderror">{{ old('address', $student->address) }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center">
                        <div class="flex items-center">
                            <input type="checkbox" name="has_transport" id="has_transport" value="1" {{ old('has_transport', $student->has_transport) ? 'checked' : '' }}
