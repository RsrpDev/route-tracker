...
@section('title', 'Gestión de Estudiantes - Route Tracker')
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600">Administración de estudiantes del sistema de transporte escolar</p>
            </div>
        ...
        <div class="px-6 py-4">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Nombre, apellido, cédula..." class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
                <div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <div class="text-sm text-gray-500">{{ $student->identity_number ?? 'Sin cédula' }}</div>
                                    </div>
                                </div>
                            </td>
                                ...
                                <div class="flex space-x-2">
                                    ...
                                    @if($student->status === 'active')
                                        <form action="{{ route('admin.students.suspend', $student) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-yellow-600 hover:text-yellow-900" onclick="return confirm('¿Estás seguro de suspender este estudiante?')">
                                                Suspender
                                            </button>
                                        </form>
                                    @elseif($student->status === 'inactive')
                                        <form action="{{ route('admin.students.activate', $student) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="text-green-600 hover:text-green-900" onclick="return confirm('¿Estás seguro de activar este estudiante?')">
                                                Activar
                                            </button>
                                        </form>
                                    @endif
